<template>
    <div class="flex flex-shrink-0 items-center w-20 h-6 border p-0.5" :class="{ 'border-2 border-blue-500': isSelected, 'border-x-slate-400': store.activeCell.cellIndex === index && store.activeCell.rowIndex > rowIndex, 'border-y-slate-400': store.activeCell.rowIndex === rowIndex && store.activeCell.cellIndex > index }" @click="cellClick">
